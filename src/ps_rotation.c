...
void	ps_push_swap(t_pile *a, t_pile *b)
{
	...
	while ((*pile_get(a, a->size - 1) == *pile_get(a, 0) + 1 ||
				*pile_get(a, a->size - 1) == 1) && !ps_is_sorted(a))
		ps_r(a, "ra\n");
	ps_push_swap(a, b);
...
{
	while (*pile_get(a, a->size - 1) <= top_value &&
			*pile_get(a, a->size - 1) != 1)
	{
		if (*pile_get(a, a->size - 1) == *pile_get(a, 0) + 1)
			ps_r(a, "ra\n");
		else
			ps_p(b, a, "pb\n");
	}
	if (ps_get_min(b) == *pile_get(a, 0) + 1)
		ps_push_swap(a, b);
}
...
void	ps_roll_back_and_forth(t_pile *a, t_pile *b, int top_value)
{
	...
	while (*pile_get(a, a->size - 1) <= top_value &&
			*pile_get(a, a->size - 1) != 1)
	{
		if (*pile_get(a, a->size - 1) >= average)
		{
			ps_r(a, "ra\n");
			k++;
		}
	...
	i = -1;
	while (++i < k)
		if (*pile_get(b, b->size - 1) != ps_get_max(b))
			ps_rrr(a, b, "rrr\n");
		else
			ps_r_r(a, "rra\n");
	if (ps_get_min(b) == *pile_get(a, 0) + 1 && k > 0)
		ps_push_swap(a, b);
}
