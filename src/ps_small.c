...
void	ps_small_sort(t_pile *p, int is_a)
{
	...
	while (--i > 0)
	{
		if (*pile_get(p, p->size - 1 - is_a) == ps_get_min(p) &&
				*pile_get(p, p->size - 2 + is_a) == ps_get_max(p))
			is_a ? ps_r(p, "ra\n") : ps_r(p, "rb\n");
		temp = -2 * is_a + 1;
		if (*pile_get(p, i - 1) * temp > *pile_get(p, i) * temp)
		{
...
void	ps_small_split(t_pile *a, t_pile *b)
{
	...
	while (ps_get_min(a) < average)
	{
		...
		{
			if (*pile_get(a, 0) + 1 == *pile_get(a, a->size - 1))
			{
				ps_small_rotate_a(a);
				if (ps_is_sorted(a))
					return ;
